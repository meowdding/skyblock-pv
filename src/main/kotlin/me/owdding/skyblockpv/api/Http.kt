...
import tech.thatgravyboat.skyblockapi.utils.http.HttpResponse
...
object Http {
    ...
    private fun connect(
        ...
        require(url.startsWith("https://") || allowHttp) { "Only HTTPS is supported" }
    ...
    suspend fun <T : Any> head(
        ...
        val response = connect(url, timeout, queries, headers, allowHttp) {
            HEAD()
        }
    ...
    suspend inline fun <reified T : Any> getResult(
        ...
        get(url = url, queries = queries, timeout = timeout, headers = headers, allowHttp = allowHttp) {
            if (isOk) {
    ...
    suspend fun <T : Any> post(
        ...
        val connection = connect(url, timeout, queries, headers, allowHttp) {
    ...
    suspend fun <T : Any> post(
        ...
        return post(url, timeout, queries, newHeaders, gson.toJson(body), allowHttp, handler)
    }

    /**
     * This will perform a PUT request on a provided url and return a response that is processed by a provided handler.
     *
    ...
    suspend fun <T : Any> put(
        ...
        val newHeaders = headers.toMutableMap()
        newHeaders["Content-Type"] = "application/json"
        return put(url, timeout, queries, newHeaders, gson.toJson(body), allowHttp, handler)
    }
}
