...
class RaritiesMuseumData(gameProfile: GameProfile, profile: SkyBlockProfile? = null) : AbstractMuseumItemScreen(gameProfile, profile) {
    override fun getInventories(): List<Display> {
        return getMuseumData().groupBy { ItemAPI.getItem(it.id).getData(DataTypes.CATEGORY) }
            .map { (_, items) ->
                items.asSequence().sortedWith(
            ...
            }.flatten().map { Displays.inventoryBackground(9, 6, Displays.padding(2, it)) }

    }

    override fun getIcons() = getMuseumData().groupBy { ItemAPI.getItem(it.id).getData(DataTypes.CATEGORY) }
        .map { (_, items) -> mutableListOf<ItemStack>().rightPad(items.chunked(54).size, ItemAPI.getItem(items.first().id)) }
        .flatten()
        .map { it.copy() }
