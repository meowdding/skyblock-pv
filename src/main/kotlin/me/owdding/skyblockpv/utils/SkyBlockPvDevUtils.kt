...
internal fun debugToggle(path: String, description: String = path): DebugToggle {
    return DebugToggle(SkyBlockPv.id(path), description, SkyBlockPvDevUtils)
}

@Module
object SkyBlockPvDevUtils : DevUtils() {
    private val path = System.getProperty("skyblock_pv.property_path")?.let { Path(it) } ?: McClient.config.resolve("skyblock_pv.properties")
